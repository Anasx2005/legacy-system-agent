...
def test_writer_canonicalizes_a_unique_bare_evidence_filename(monkeypatch, tmp_path):
    evidence_directory = tmp_path / "evidence"
    model_repository_directory = tmp_path / "model-repo"
    (evidence_directory / "strategy").mkdir(parents=True)
    (evidence_directory / "motivation").mkdir(parents=True)
    ...
    element_data = valid_strategy_element().model_dump()
    element_data["evidence"] = [
        {
            "source_type": "strategy_document",
            "locator": "modernisation-plan.md#Customer self-service",
        }
    ]
    element = ModelElement.model_validate(element_data)
    output_file = persist_strategy_element(element)

    assert '"locator": "strategy/modernisation-plan.md#Customer self-service"' in output_file.read_text(encoding="utf-8")
